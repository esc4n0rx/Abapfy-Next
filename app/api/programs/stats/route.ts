...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: programsByType } = await supabaseAdmin
      .from('abap_programs')
      .select('type')
      .eq('user_id', decoded.userId);
    ...
    programsByType?.forEach(program => {
      typeDistribution[program.type] = (typeDistribution[program.type] || 0) + 1;
    });

    usageData?.forEach(program => {
    ...
    const stats = {
      totalPrograms: totalCount?.length || 0,
      programsByType: typeDistribution,
